...
function convertToRomanNumerals(num) {
  ...
  const digitOnes = num % 10;
  const digitTens = Math.floor(num / 10);
  return numbersTeen[digitTens] + numbersOne[digitOnes];
}
...
function getBalanceIndex(arr) {
  for (let i = 0; i < arr.length; i += 1) {
    ...
    for (let w = i + 1; w < arr.length; w += 1) {
      sumRight += arr[w];
    }
    if (sumLeft === sumRight) {
      return i;
...
function getSpiralMatrix(size) {
  ...
  while (startCol <= endCol && startRow <= endRow) {
    ...
    endCol -= 1;
    for (let w = endCol; w >= startCol; w -= 1) {
      result[endRow][w] = counter;
      counter += 1;
    }
    endRow -= 1;
    for (let q = endRow; q >= startRow; q -= 1) {
      result[q][startCol] = counter;
      counter += 1;
    }
    startCol += 1;
  }
